...
async function my(prompt, customId, link = null) {
    try {
...
module.exports = {
    ...
    onStart: async ({ message: { reply: r }, args: a, event: { senderID: s, messageReply }, commandName }) => {
        ...
        if (messageReply?.attachments?.[0]?.type === "photo") {
            ...
            res = await my("Merci pour l'image ! Que voulez-vous savoir à propos de cette image ?", s, messageReply.attachments[0].url);
        } else {
            // Sinon, traitement normal du texte
            res = await my(a.join(" ") || "hello", s);
        }
    ...
    onReply: async ({ Reply: { s, commandName }, message: { reply: r }, args: a, event: { senderID: x } }) => {
        ...
        const res = await my(a.join(" ") || "hello", s);
        const { messageID: m } = await r(res);
