...
const services = [
  { url: 'https://markdevs-last-api-as2j.onrender.com/gpt4', param: { prompt: 'prompt', uid: 'uid' }, isCustom: true }
];

async function callService(service, prompt, senderID) {
  if (service.isCustom) {
    try {
      const apiUrl = `${service.url}?prompt=${encodeURIComponent(prompt)}&uid=${encodeURIComponent(senderID)}`;
      const response = await axios.get(apiUrl);
      return response.data.gpt4 || response.data;
    } catch (error) {
...
module.exports = {
  ...
  langs: {
    en: {
      final: "",
      header: " ðŸ¤– ð—–ð—œð—–ð—œ ð—”ð—¦ð—¦ð—œð—¦ð—§ð—”ð—¡ð—§\nâ”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”",
